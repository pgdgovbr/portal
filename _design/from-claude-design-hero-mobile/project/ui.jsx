...
const TopNavUserMenu = ({ user }) => {
  const { open, setOpen, ref } = useDropdown();
  return (
    <div ref={ref} style={{ position: "relative" }}>
      <button className="tn-user" onClick={() => setOpen(o => !o)} aria-haspopup="menu" aria-expanded={open}>
        <span className="av av-sm" style={{ background: "var(--c-primary)", color: "white" }}>{user.initials}</span>
      ...
      {open && (
        <div role="menu" style={{
          ...
          <div style={{ padding: "10px 12px 12px", borderBottom: "1px solid var(--c-divider)", marginBottom: 4 }}>
            <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
              <span className="av av-md" style={{ background: "var(--c-primary)", color: "white" }}>{user.initials}</span>
              <div style={{ minWidth: 0, flex: 1 }}>
                <div style={{ fontWeight: 600, fontSize: 13.5, color: "var(--c-ink)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{user.name}</div>
                <div style={{ fontSize: 11.5, color: "var(--c-muted)" }}>{user.role}</div>
              </div>
            </div>
          </div>
          {[
            { lbl: "Meu perfil",   icon: "users",    href: "#perfil" },
            { lbl: "Preferências", icon: "settings", href: "#prefs" },
            { lbl: "Notificações", icon: "bell",     href: "#notif" },
          ].map((it, i) => (
            <a key={i} href={it.href} role="menuitem" style={{
              display: "flex", gap: 10, alignItems: "center",
              padding: "9px 12px", borderRadius: "var(--r-sm)",
              fontSize: 13.5, color: "var(--c-ink-2)", textDecoration: "none",
            }} onMouseEnter={e => e.currentTarget.style.background = "var(--c-surface-2)"}
              onMouseLeave={e => e.currentTarget.style.background = "transparent"}>
              <Icon name={it.icon} size={15} className="" />
              {it.lbl}
            </a>
          ))}
          <div style={{ height: 1, background: "var(--c-divider)", margin: "4px 0" }} />
          <a href="#trocar-persona" role="menuitem" style={{
            display: "flex", gap: 10, alignItems: "center",
            padding: "9px 12px", borderRadius: "var(--r-sm)",
            fontSize: 13.5, color: "var(--c-ink-2)", textDecoration: "none",
          }} onMouseEnter={e => e.currentTarget.style.background = "var(--c-surface-2)"}
            onMouseLeave={e => e.currentTarget.style.background = "transparent"}>
            <Icon name="refresh" size={15} className="" />
            Trocar de persona (demo)
          </a>
          <a href="#logoff" role="menuitem" style={{
            display: "flex", gap: 10, alignItems: "center",
            padding: "9px 12px", borderRadius: "var(--r-sm)",
            fontSize: 13.5, color: "var(--c-danger)", textDecoration: "none", fontWeight: 500,
          }} onMouseEnter={e => e.currentTarget.style.background = "var(--c-danger-soft)"}
            onMouseLeave={e => e.currentTarget.style.background = "transparent"}>
            <Icon name="arrowL" size={15} className="" />
            Sair
          </a>
        </div>
      )}
...
Object.assign(window, {
  Icon, TopNav, TopNavUserMenu, StatusBadge, NotaBadge, ModalidadeBadge,
