...
> Para o **Claude Code**. Complemento do [`handoff-landing-v2.md`](./handoff-landing-v2.md). Esta entrega cobre **apenas a versão mobile** da Landing pública. O conteúdo, tokens, marca e regras já decididas continuam valendo — aqui o foco é o layout em telas estreitas (≤ 720px) e as transformações específicas que ele exige.
...
**Desktop:** 2 colunas — texto à esquerda + browser frame com `ScreenRegistrar` em `scale(0.5)` à direita.
**Mobile:** 1 coluna.

- H1 usa `font-size: clamp(28px, 8.4vw, 36px)` pra acompanhar o viewport.
- Lede em 15.5px.
- CTAs primário + outline empilhados em coluna, full-width.
- O browser frame com `ScreenRegistrar` é **substituído por um screenshot estático** (`screenshots/mobile-servidor.png` no protótipo). Implementar o `ScreenRegistrar` real em scale dentro de viewport mobile não vale o custo de LCP; usar imagem.
